...
/**
 * Composite metric band: one SVG with all four metric cards — value, delta
 * arrow, 30-run sparkline — colored green/gray/red by delta. Committed to the
 * baseline branch as badges/metric-band-{light,dark}.svg and embedded in the
 * comment via <picture> with prefers-color-scheme variants. Deterministic:
 * same series in, byte-identical SVG out (D7).
 */
...
export function renderMetricBandSvg(series: HistoryPoint[], theme: Theme): string {
  ...
  const cards = METRIC_KEYS.map((key, i) => {
    const values = series.map((p) => p.metrics[key]).slice(-30);
    ...
    const spark = sparklinePath(values, x + 14, 58, CARD_W - 28, 22);
    return [
