...
export function compareFileCoverage(
    ...
    if (isNewFile) {
      ...
      const fileMetrics = {
        branches: {
          ...
          symbol:
            pr[file].branches.pct > base[file].branches.pct
              ? '⬆️'
              : pr[file].branches.pct < base[file].branches.pct
                ? '⬇️'
                : '➖',
        ...
        functions: {
          ...
          symbol:
            pr[file].functions.pct > base[file].functions.pct
              ? '⬆️'
              : pr[file].functions.pct < base[file].functions.pct
                ? '⬇️'
                : '➖',
        ...
        lines: {
          ...
          symbol:
            pr[file].lines.pct > base[file].lines.pct
              ? '⬆️'
              : pr[file].lines.pct < base[file].lines.pct
                ? '⬇️'
                : '➖',
        ...
        statements: {
          ...
          symbol:
            pr[file].statements.pct > base[file].statements.pct
              ? '⬆️'
              : pr[file].statements.pct < base[file].statements.pct
                ? '⬇️'
                : '➖',
