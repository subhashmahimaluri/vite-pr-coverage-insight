...
export function renderFixPlan(report: CoverageReport): string {
  ...
  if (survivors.length > 0) {
    parts.push(
      ...
      'that coverage alone never forced you to write.',
      ''
    );
    survivors.slice(0, 15).forEach((m, i) => {
      parts.push(
        `### ${i + 1}. \`${m.mutator}\` at \`${m.file}:${m.line}\``,
        '',
        '**Prompt for your AI assistant:**',
        '',
        '```text',
        `Write a test that kills this surviving mutant in \`${m.file}\` (line ${m.line}):`,
        `the ${m.mutator} mutator changed the code${m.replacement ? ` into \`${m.replacement}\`` : ''}`,
        'and the whole test suite still passed. Add the missing assertion that fails',
        'under the mutation and passes on the original code. Do not modify the source.',
        '```',
        ''
      );
    });
    if (survivors.length > 15) {
      parts.push(`_…and ${survivors.length - 15} more — see coverage-report.json (mutation)._`, '');
    }
  }
